...
struct DebloatTweak: Identifiable {
    let id: String
    let name: String
...
struct DebloatCategory: Identifiable {
    let id: String
    let name: String
    let description: String
    let icon: String
...
class DebloatManager: ObservableObject {
    ...
    func toggle(_ tweak: DebloatTweak) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let success: Bool
            if applyNow {
                success = self?.execute(tweak.applyCommands, privileged: tweak.applyPrivileged) ?? false
            } else {
                success = self?.execute(tweak.revertCommands, privileged: tweak.revertPrivileged) ?? false
            }
    ...
    func applyAll(in category: DebloatCategory) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            var applied = 0
            for tweak in unapplied {
                let ok = self?.execute(tweak.applyCommands, privileged: tweak.applyPrivileged) ?? false
                let newState = tweak.detect()
                DispatchQueue.main.async {
                    self?.tweakStates[tweak.id] = newState
                    self?.busyTweaks.remove(tweak.id)
            ...
            DispatchQueue.main.async {
                self?.toast("Applied \(applied)/\(unapplied.count) \(category.name) tweaks")
            }
        }
    }
    ...
    func revertAll(in category: DebloatCategory) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                self?.toast("Reverted \(reverted)/\(applied.count) \(category.name) tweaks")
            }
        }
    }
