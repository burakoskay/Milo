...
**Exit gate: [x] Completed 2026-07-26 at remote commit `01c2ad0535a0bf7aa83e4d8e50eb9247327801f3`.** A brand-new clone with no ignored `Secrets.swift` remained byte-clean after canonical project regeneration and reproduced project SHA-256 `f1cabf96ba1c876eee873de4b807f49e8314af2fcf522b10ac73d12b34a2e549`. Fully isolated scratch/DerivedData runs passed root red-team tests 16/16, MiloKit 17/17, Pro 23/23, and Lite 8/8 including its UI launch test. Strict SwiftLint found 0 violations in the clean clone's 58 tracked Swift files; forbidden `try?`, `as!`, and force-unwrap scans were empty. Clean unsigned Debug and Release builds emitted no app/compiler warning diagnostics. Release produced Universal `arm64`/`x86_64` Pro, Lite, helper, and every embedded Sparkle executable with macOS 13.0 minimum; Pro retained the expected helper/launch-daemon/Sparkle layout and Lite remained a four-file framework-free bundle. Xcode 27 beta 4 still emits the already tracked internal `DVTAssertions` launch-session warning between passing test bundles; this is an Apple beta-toolchain diagnostic, not an application/compiler warning, and remains a mandatory next-seed/RC retest. This closes build coherence only and makes no behavioral release-readiness claim.
...
2. [ ] Mark UI models `@MainActor`; replace shared mutable singletons with injected actors/services.
   - [x] **Process rule dependency injection completed 2026-07-26:** `ProcessManager` now receives its cloud-rule service through its initializer and scan logic no longer resolves `CloudSignatureManager.shared` internally. The default remains at the explicit composition boundary while the remaining runtime singletons are migrated, so item 2 is not complete.
3. [ ] Add cancellation, generation, deadlines, and resource caps to every asynchronous/subprocess operation.
   ...
   - [x] **Allowlisted command deadline completed 2026-07-26:** `CommandRunner` now uses an owned no-shell POSIX runner that atomically creates a private process group, applies a 15-second default deadline with a 60-second hard maximum, propagates Swift task cancellation, and terminates the complete group on timeout, cancellation, I/O failure, or output overflow. Cleanup is itself bounded, all termination outcomes are typed, and real-process tests cover normal exit, hung descendants, cancellation, and streaming overflow. The synthetic self-test launcher also uses the same bounded runner and has no direct `Foundation.Process` path. The administrator-prompt replacement remains Phase 7 work, so item 3 is not complete.
