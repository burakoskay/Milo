...
**Exit gate:** clean protected migration branches, immutable baseline tag, decision records, product/capability matrix, and reviewed protocol/threat-model documents.

### Phase 1 — Restore a coherent, warning-free build

1. [ ] Update to the current Xcode 27 beta, verify its signed Apple distribution, select it explicitly with `DEVELOPER_DIR`, record Xcode/SDK/Swift build identifiers, and rerun every baseline build/test. Never relabel beta 1 evidence as beta 4 evidence; repeat again at RC/GM.
2. [ ] Add the canonical Xcode workspace/project while retaining SwiftPM domain packages.
